#!/usr/bin/env python3
"""
ICL Vault Prediction - Tight Chamber LightGBM Model (lgb-27f-756c)

Self-contained training script that:
1. Loads training_data.csv
2. Engineers all 24 existing gestalt features + 3 new tight-chamber features (27 total)
3. Trains LightGBM with balanced class weights (matching lgb-24f regularized config)
4. Saves to models/archives/lgb-27f-756c/

Parent: lgb-24f-756c (LightGBM, 24f, balanced, regularized)
"""
...
ARCHIVE_DIR = PROJECT_ROOT / "models" / "archives" / "lgb-27f-756c"
...
# ── lens size classifier (LightGBM) ─────────────────────────────────────
def train_lens_size_model(X, y):
    print("\n" + "=" * 70)
    print("TRAINING LENS SIZE CLASSIFIER (LightGBM, balanced)")
    print("=" * 70)

    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)

    # Regularized config from lgb-24f-756c README
    params = dict(
        n_estimators=150,
        max_depth=4,
        num_leaves=15,
    ...
    for fold_i, (train_idx, val_idx) in enumerate(cv.split(X_scaled), 1):
        ...
        y_fold_pred = fold_model.predict(X_val)
    ...
    print(f"  Mean max prob:  {max_probs.mean():.2f}")
    print(f"  At 100%:        {(max_probs > 0.999).sum()}/{len(max_probs)}")
    print(f"  Below 80%:      {(max_probs < 0.80).sum()}/{len(max_probs)}")
    ...
    for i, idx in enumerate(indices, 1):
        print(f"  {i:2d}. {feature_names[idx]:30s}: {importances[idx]}")

    return model, scaler, mean_acc, std_acc


# ── vault regressor (same GB regressor as other models) ──────────────────
...
def main():
    print("\n" + "=" * 70)
    print("ICL VAULT - TIGHT CHAMBER LightGBM (lgb-27f-756c)")
    print("=" * 70)
