...
| Metric | Value |
|---|---|
| Lens size accuracy | 73.2% (5-fold CV) |
...
From `model_performance_history.json`, the final two runs on Feb 8:
- Run 1 (no gestalts): 65.3% lens accuracy, 130.3 µm vault MAE
- Run 2 (with gestalts): **73.2% lens accuracy, 128.3 µm vault MAE**

Gestalt features added +7.9% lens accuracy on the same data.
...
- **Selection:** Compared against `RandomForestClassifier(n_estimators=150, max_depth=5, min_samples_split=5, class_weight='balanced')` — GradientBoosting won on CV accuracy
...
- **Selection:** Compared against `RandomForestRegressor(n_estimators=100, max_depth=4)` and `Ridge(alpha=1.0)` — GradientBoosting won on CV MAE

### Cross-Validation
- 5-fold, `KFold(n_splits=5, shuffle=True, random_state=42)`
- Both models evaluated via `cross_val_score`, best selected, then retrained on full dataset
- Detailed evaluation via `cross_val_predict` for classification report and confusion matrix

### Preprocessing
- `StandardScaler` fitted independently for lens and vault models
- Lens_Size target: absolute value taken (fixes negatives like -12.6 → 12.6), converted to string for classification
- Outlier filter: `0 < Lens_Size < 20`
...
Cutoffs were derived from `gestalt_discovery.csv` and `gestalt_discovery_by_size.csv` — audit spreadsheets sorted by actual surgical outcome vault:

- Low-vault outliers (<200 µm) cluster around `AC_shape_ratio < 55`
- High-vault outliers (>900 µm) cluster around `ACD_internal > 3.8` or `ICL_Power < -14`

These patterns informed the thresholds in the binary flags and bucket boundaries.

## Deploy

This model is currently live in `models/current/`. To redeploy:

```bash
cp models/archives/gestalt-24f-756c/*.pkl models/current/
git add models/current/*.pkl
git commit -m "Deploy gestalt-24f-756c to production"
